...
REQUIRE_VOLUME_INCREASE = env_bool("REQUIRE_VOLUME_INCREASE", True)
...
def is_bullish_signal(candles: List[List[float]]) -> Optional[Dict[str, float]]:
    ...
    current_candle_volume_usdt = float(volumes[idx] * closes[idx])
    previous_candle_volume_usdt = float(volumes[idx - 1] * closes[idx - 1])

    if current_candle_volume_usdt < MIN_CANDLE_VOLUME_USDT:
        return None

    if REQUIRE_VOLUME_INCREASE and current_candle_volume_usdt <= previous_candle_volume_usdt:
        return None

    volume_increase_ratio = (
        current_candle_volume_usdt / previous_candle_volume_usdt
        if previous_candle_volume_usdt > 0
        else 0.0
    )
    ...
    if stoch_ok and macd_ok:
        return {
            "price": closes[idx],
            "current_candle_volume_usdt": current_candle_volume_usdt,
            "previous_candle_volume_usdt": previous_candle_volume_usdt,
...
class BotRunner:
    ...
    def build_message(self, exchange_id: str, symbol: str, r: Dict[str, float]) -> str:
        candle_time = time.strftime(
            "%Y-%m-%d %H:%M UTC",
            time.gmtime(r["candle_time"] / 1000),
        )
...
""".strip()
    ...
    async def scan_exchange(self, exchange_id: str) -> int:
        ...
        try:
            ...
            tasks = [
                self.analyze_symbol(exchange, exchange_id, symbol)
    ...
    async def run(self):
        ...
        await self.send(
            "✅ Bot started: Stochastic RSI + MACD + 4H candle volume increase, no pandas"
        )
